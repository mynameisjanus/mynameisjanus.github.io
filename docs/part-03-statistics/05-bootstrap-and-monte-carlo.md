...
Resampling is the way out: let the data stand in for the distribution that generated it, and simulate the sampling variability you cannot derive. This lesson builds the bootstrap from its iid form to the block form that respects the serial dependence lesson three documented, assembles a reusable Sharpe-interval pipeline, turns Monte Carlo loose on drawdown distributions, and closes with permutation tests — plus the honest list of places where resampling quietly lies. The strategy under the microscope is still lesson four's twelve-month momentum rule.
