...
!!! warning "The sample size that makes a mean test valid is not the sample size that makes a tail estimate valid, and only the first one is ever checked"
    Both columns above are the same estimator on the same data under the same theorem. A backtest that reports a t-statistic and a $99.9\%$ VaR is relying on the CLT twice, at two levels whose convergence rates differ by a factor of thousands in sample size, and reporting one significance discussion for both. The rule that follows is narrow and cheap: the CLT licenses inference about the centre of a distribution at the sample sizes finance has, and licenses nothing about the extreme quantiles at any sample size finance has. Tail quantities need a method built for tails — the peaks-over-threshold fit of [Extreme Value Theory](../part-18-quant-finance-applications/14-extreme-value-theory.md) — rather than a normal approximation extrapolated past where it was ever checked.
