...
Two things are worth taking from the multivariate version of the question. [Correlation Matrices](../part-06-multivariate-probability/03-correlation-matrices.md) shows that the move from $0.314$ to $0.922$ is better read as a fixed budget of eigenvalues concentrating into one direction than as an average rising, because the budget is $N$ in every regime and only its spread can change. And [Conditional Gaussian Distributions](../part-06-multivariate-probability/06-conditional-gaussian.md) shows that no joint normal can produce the rise at all: conditioning on a bad market provably *lowers* the average pairwise correlation, so the regime dependence is not a parameter the standard model estimated badly, it is a shape the standard model does not have.
