...
This is the same tradeoff, in the same shape, as the bandwidth question on [Probability Density Functions](04-probability-density-functions.md), where shrinking $\delta$ around a point drives the bias down and the noise up in exactly this pattern. The two are not duplicates of each other; they are the one-variable and two-variable faces of a single fact, which is that a density at a point — conditional or not — is defined by a limit that no finite sample can take. Anywhere a density is estimated, an $\varepsilon$ has been chosen, and choosing it is choosing which of the two errors to prefer.
