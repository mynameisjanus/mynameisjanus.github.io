...
Nothing new is defined, but the arithmetic is better conditioned. Deep in the tail $F$ is a number like $0.9999$ and $S$ is $10^{-4}$, and subtracting the first from one destroys the precision the second retains. Every heavy-tail description is a statement about $S$ — a power law is $S(x)\sim cx^{-\alpha}$, and comparing that decay against an exponential one is how [Heavy-Tailed Returns](../part-18-quant-finance-applications/12-heavy-tailed-returns.md) and [Extreme Value Theory](../part-18-quant-finance-applications/13-extreme-value-theory.md) distinguish tail regimes.
