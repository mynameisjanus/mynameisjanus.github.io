...
For a normal with known variance the score equations return the sample mean; for the normal's variance they return the sample variance with divisor $n$, not $n-1$, which is where the maximum likelihood estimator's finite-sample bias enters and why [Bias and Variance](../part-10-statistics-foundations/07-bias-and-variance.md) has a correction to discuss at all. For a Student-$t$ there is no closed form, and the equations are solved numerically. The inner loop used below to fit its location and scale — reweighting each observation by $(\nu+1)/(\nu+(x_i-m)^2/s^2)$ and recomputing a weighted mean and variance — is not an ad hoc iteration but expectation-maximization for a Student-$t$ written as a scale mixture of normals, with the weight playing the role of the imputed latent scale; its convergence rate and the standard error it does *not* supply are [The EM Algorithm](../part-17-statistical-computing/03-em-algorithm.md). The important structural point is that the same derivative that locates the maximum also measures how sharp it is, and sharpness is precision.
