...
Read against the previous section, this is the page's real argument. The just-identified moment estimator of $\nu$ was silently wrong by a factor of $1.56$ and reported a shrinking standard error while it happened. The overidentified version of the same idea, applied to the same kind of data, announces a specification failure with a statistic four orders of magnitude past its critical value. Same method, same arithmetic, and the difference is entirely whether $q-p$ was allowed to be positive. Turning that statistic into a formal decision with a stated error rate is [Likelihood Ratio Tests](../part-12-hypothesis-testing/06-likelihood-ratio-tests.md), which also measures what the stated rate is worth: on breach counts the Wald, score and likelihood-ratio statistics read $2.09$, $4.95$ and $3.56$ against one critical value of $3.8415$, so at $250$ observations the verdict depends on which of the three was typed. Here it is a residual the estimator produced without being asked.
