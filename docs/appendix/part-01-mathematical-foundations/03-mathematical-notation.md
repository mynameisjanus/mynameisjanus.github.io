...
4. **The prefactor names the correction.** Dividing by $n-1$ rather than $n$ compensates for having estimated the two means from the same data — the bias correction derived in [Bias and Variance](../part-10-statistics-foundations/05-bias-and-variance.md).
