...
Its Sharpe of 0.377 loses to **1/N's 0.450** — a portfolio that requires no estimation, no linear algebra, and no rebalancing at all. The same crossover is measurable with no market in it, because $\Sigma^{-1}\mathbf{1}$ is also the formula for combining forecasters: [Model Averaging](../appendix/part-14-model-selection/05-model-averaging.md) finds equal weights winning once the members outnumber roughly a tenth of the observations. The robustness sweep says this is not a quirk of one window. Across five configurations MVO ranges from **−0.015 to 0.464** while 1/N ranges from 0.400 to 0.482, and MVO beats it exactly once, on the widest universe with the longest window, where it does so at a volatility that would have liquidated the account long before the Sharpe was collected. Shortening the estimation window makes it worse in the way the theory predicts: at T = 63, with seven observations per asset, MVO collapses to 0.044.
...
At 504 observations for nine assets the estimator shrinks by **1.85%**. It is not being timid — it is being correct: with 56 observations per asset the sample covariance is already well-conditioned, and the optimal shrinkage toward a scalar target really is almost nothing. Ledoit–Wolf is doing its job perfectly and the job is nearly vacuous here. The intensity rises to 9.69% when the window shrinks to seven observations per asset, exactly as the theory says it should, and even there it cannot rescue MVO (Sharpe −0.160). [Covariance Matrices](../appendix/part-06-multivariate-probability/02-covariance-matrices.md) derives why the intensity is essentially a function of $N/T$, and reruns this experiment on simulated data where the truth is known: shrinkage improves the minimum-variance portfolio at every sample size and buys the tangency portfolio 0.015 of Sharpe, against the 0.81 that substituting the true $\mu$ buys with the unshrunk matrix left exactly as it was.
